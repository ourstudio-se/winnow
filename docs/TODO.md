...
- [x] Frontend auth handling: central `ensureOk` gate in `api.ts` — 401 redirects to the configured login URL, 403 flips the app to a full-screen "no permission to view telemetry" page (`components/auth-error.tsx`, mounted via `onAuthFailure` listener in `layout.tsx`)
- [x] Auth error page has a Retry button: 403 (`forbidden`) retry clears the latch and remounts the views (they refetch on mount); no more permanently bricked UI on a stray upstream 403
- [x] Login-loop guard: `redirectToLogin` appends `winnow_relogin=1` to the return URL inside the `{winnow_return_url}` substitution; if a 401 arrives while that marker is on the current URL, the app shows a "Signing in didn't stick" page (with Retry → clears marker, new login round) instead of redirecting forever. The marker is stripped via `history.replaceState` on the first successful API response so it doesn't linger in copied links.
- [x] `GET /api/v1/ui-config` — unauthenticated bootstrap endpoint (classified `.static` in worker.zig: api role, no auth gate) returning `{login_url, logout_url}` from the api authorizer's auth block; handler in `api.zig` (`handleUiConfig`)
- [x] Frontend config plumbing: `lib/config.ts` fetches ui-config once (module-cached promise, degrades to nulls on failure); `useUiConfig()` hook
- [x] Logout button at the sidebar bottom (`components/logout-button.tsx`) — links to `logout_url`, collapsed-aware with tooltip, hidden when no logout URL is configured
- [ ] Verify: with auth enabled, `curl /api/v1/ui-config` returns the URLs without a token; `/api/v1/traces/search` without a token → 401 and the UI redirects to login; a token failing authorization → 403 page; logout button appears and hits logout_url
- [x] `login-server` dev tool (`scripts/login-server.py`, `packages.login-server`, in devshell) for the cookie auth strategy: `GET /login?return_to=…` mints an RS256 JWT with the shared `.dev-jwks` key and sets it as a cookie (`--cookie-name`, default `jwt`; `--sub/--ttl/--scopes/--claims` like generate-token), 302s back to `return_to`; `GET /logout` clears the cookie. Default port 3005 (matches winnow.kdl login/logout URLs). Verified: login sets a cookie that validates against the dev key (kid `winnow-dev`), logout expires it. Stage `scripts/login-server.py` before re-entering the devshell.
- [x] `pnpm install` failed on unapproved build scripts (`ERR_PNPM_IGNORED_BUILDS` for `msw`, a transitive dep of the shadcn CLI present since the first frontend commit; pnpm ≥10 blocks unapproved postinstalls). Fixed: `allowBuilds: msw: false` in `frontend/pnpm-workspace.yaml` (was an unfilled placeholder) — the script only provisions MSW's browser service-worker artifacts, unused here. `pnpm build` and `zig build -Dforce-frontend` work again; stage `pnpm-workspace.yaml` (it was untracked).
...
## General-Purpose Metrics Module (deferred, Sept 2026)

- [x] Evaluated backends for an optional time-series metrics sink (host metrics, request latencies): VictoriaMetrics, GreptimeDB, Prometheus 3.x push sink, ClickHouse — full write-up in ARCHITECTURE.md "General-Purpose Metrics (Deferred)"
- [x] Verified Quickwit upstream is building a dedicated parquet/DataFusion metrics engine (`quickwit-parquet-engine`, `quickwit-datafusion`, `quickwit-compaction` crates; OTLP metrics receiver; ADRs 001–003) — Datadog rebuilding Husky-style storage in the OSS repo; active commits through Sept 2026 but no public query API yet
- [x] Decision: hold off on the feature entirely; we stay all-in on Quickwit rather than adding a second storage backend
- [ ] Watch upstream: adopt once OTLP metrics ingest + a public query surface (SQL/PromQL) + docs land in a tagged Quickwit release (adoption criteria in ARCHITECTURE.md)
