...
fn handleServiceGraph(
    ...
    const connector_query_json = try std.fmt.allocPrint(arena,
        \\{{"query":"{s}","max_hits":0,"aggs":{{"by_client":{{"terms":{{"field":"client","size":200}},"aggs":{{"by_server":{{"terms":{{"field":"server","size":200}},"aggs":{{"total_calls":{{"sum":{{"field":"calls"}}}},"total_errors":{{"sum":{{"field":"errors"}}}},"conn_type":{{"terms":{{"field":"connection_type","size":5}}}},"by_server_fp":{{"terms":{{"field":"server_fingerprint","size":100}}}}}}}}}}}}}}}}
    , .{edges_index_query});
